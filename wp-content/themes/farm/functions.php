...
function farm_setup() {
	add_theme_support( 'custom-header', apply_filters( 'farm_custom_header_args',
		array(
...
function farm_customize_register( $wp_customize ) {
	...
	$wp_customize->add_section( 'farm_staff', array(
	...
	for ( $i = 1; $i < 9; $i++ ) {
		$wp_customize->add_setting( 'farm_staff_member' . $i, array(
			'sanitize_callback' => 'farm_sanitize_select',
		) );

		$wp_customize->add_control( 'farm_staff_member' . $i, array(
			'type' => 'select',
