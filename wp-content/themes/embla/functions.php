...
if ( ! function_exists( 'embla_setup' ) ) {
	...
	function embla_setup() {
		...
		add_theme_support( 'custom-header', apply_filters( 'embla_custom_header_args',
			array(
				'default-image'          => false,
		...
		add_theme_support( 'editor-color-palette', '#0073AA' );
...
/*
Custom CSS for the icon and accent colors.
*/
function embla_customize_css() {
	...
	if ( get_theme_mod( 'embla_accent_color' ) !== '0073AA' ) {
		...
		echo '.social-menu li a:focus .icon, .social-menu li a:hover .icon { fill: ' . esc_attr( get_theme_mod( 'embla_accent_color' ) ) . ';} ';
