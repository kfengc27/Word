	...
	public function profile_magic_get_user_attachment($value,$key='')
	{
		...
		{ 
			...
			$attachment_html .='<span class="pm-attachment-title pm-dbfl"><a href="'.wp_get_attachment_url( $fileid ).'">'.get_the_title( $fileid ).'</a></span>';
                        if($key != '')
                        {
                        $attachment_html .='<a class="removebutton"><span onClick="pm_remove_attachment(this,\''.$key.'\','.$fileid.')" class="remove">Remove</span></a>';
                        }
		...
		{                     
                ?>
                    <span class="pm_frontend_attachment<?php if($key != '') { echo ' pm_edit_attachment'; }?>">
                        ...
                        <a class="pm_removebutton"><span onClick="pm_remove_attachment(this, '<?php echo $key ?>', <?php echo $fileid; ?>)" class="remove">Remove</span></a>
                    </span>
                <?php
                }
		unset($values);
...
<div class="pm_five_star_Banner">
    <p align="center"><?php _e("Do you like ProfileGrid? Help us  make it better…Please rate it ",'profile-magic'); ?>
        <span class="pm-star">
            ...
            <i class="fa fa-star" aria-hidden="true"></i>
            <i class="fa fa-star" aria-hidden="true"></i>
        </span><?php _e(" Stars on ",'profile-magic'); ?><a target="_blank" href="https://wordpress.org/support/plugin/profilegrid-user-profiles-groups-and-communities/reviews/?rate=5#new-post">WordPress.org</a></p>
</div>
        ...
        public function profile_magic_check_profile_access_permission($profile_id)
	{
		$current_user_id = get_current_user_id();
                ...
                $profile_user_group = $this->profile_magic_get_user_field_value($profile_id,'pm_group');
                $current_user_group = $this->profile_magic_get_user_field_value($current_user_id,'pm_group');
                ...
                switch($access_level)
                {
                    ...
                    case '2':
                        ...
                        break;
                    case '3':
                        if(is_user_logged_in() && $profile_user_group==$current_user_group)
                        {
                            $access = true;
                        }
                        break;
                    case '4':
                        if((is_user_logged_in() && $profile_user_group==$current_user_group) || ( is_user_logged_in() && isset($is_my_friend) && !empty($is_my_friend)))
                        {
                            $access = true;
                        }
                        break;
	...
	return apply_filters( 'get_edit_post_link', $link, $post->ID, $context );
        ...
        public function pg_auto_create_default_email_template($gid)
        {
            ...
            $body = "Dear {{first_name}} {{last_name}},<br /><br />Thank you for registering with us on {{site_name}}. You will receive an account activation confirmation email once your account is approved by the admin. <br /><br />Kind Regards.";
            $tmpl_data = array('tmpl_name'=>'User Registration Successful','email_subject'=>'User Registration Successful','email_body'=>$body);
            ...
            $body = "Dear {{first_name}} {{last_name}},<br /><br />Admin has suspended your user account on {{site_name}}. For now, you cannot login or access your member area.<br /><br />Kind Regards.";
            $tmpl_data = array('tmpl_name'=>'User Account Suspended','email_subject'=>'User Account Suspended','email_body'=>$body);
            $tmpl_arg = array('%s','%s','%s');
            $group_options['on_user_deactivate'] = $dbhandler->insert_row('EMAIL_TMPL',$tmpl_data,$tmpl_arg);
            
            $body = "Dear {{first_name}} {{last_name}},<br /><br />Password for your user account on {{site_name}} was successfully changed. If you did not changed it, please contact the admin immediately.<br /><br />Kind Regards.";
