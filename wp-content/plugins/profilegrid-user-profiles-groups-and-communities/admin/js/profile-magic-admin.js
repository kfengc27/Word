...
function add_group_validation()
{
	...
	jQuery('.pm_admin_label').each(function (index, element) { //Validation for number type custom field
		...
		if (value2!='' && !value2.match(regex)) {
			jQuery(this).children('.errortext').html('Please enter a valid group name.');
			jQuery(this).children('input').addClass('warning');
			jQuery(this).children('.errortext').show();
		}
...
{
var a = '<li class="pm_radio_option_field"><input type="text" value="Their answer" disabled><span class="removefield pm_remove_field" onClick="remove_pm_other_option(this)">Delete</span><input type="hidden" name="field_options[radio_option_value][]" value="chl_other" /></li>';
        ...
        custom_uploader.on('select', function() {
			...
			if(attachment['type']=='image')
			{
				...
				$(parent_div).children('.errortext').html(pm_upload_object.error_min_profile_width);
				 ...
				 $(parent_div).children('.pm_preview_img').attr('src',attachment.url);
			...
			{
				$(parent_div).children('.pm_preview_img').hide();
				$(parent_div).children('.errortext').html('This is not a valid image');
			}
