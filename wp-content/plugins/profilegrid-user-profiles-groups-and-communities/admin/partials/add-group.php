    ...
    <div class="content">
      ...
      <div class="uimrow">
        ...
        <div class="uiminput">
          ...
          <div class="uimnote"><?php _e('Turn on to hide group card on its group page.','profile-magic');?></div>
      </div>
      ...
      <div class="uimrow">
        <div class="uimfield">
          <?php _e( 'Group Leader','profile-magic' ); ?>
        </div>
        <div class="uiminput">
          ...
          <div class="uimnote"><?php _e('A group leader is mentioned on the Group page prominently. The leader also has special Privileges to moderate the group.','profile-magic');?></div>
      </div>
      <div class="childfieldsrow" id="groupleaderhtml" style=" <?php  if(!empty($row) && $row->is_group_leader==1){echo 'display:block;';} else { echo 'display:none;';} ?>">
        <div class="uimrow">
          <div class="uimfield">
            <?php _e( 'Select Leader:','profile-magic' ); ?>
          </div>
          <div class="uiminput <?php if(!empty($row) && $row->is_group_leader==1){echo 'pm_required pm_group_leader_name';}?>">
            ...
            <div class="uimnote"><?php _e('Select from existing users. Start typing for real-time search.','profile-magic');?></div>
        </div>
        <?php /*?><div class="uimrow">
          <div class="uimfield">
            <?php _e( 'Leader Rights:','profile-magic' ); ?>
          </div>
       ...
       <div class="uimrow" id="redirection">
        ...
        <div class="uiminput pm_checkbox_required">
         <ul class="uimradio">
           ...
           <div class="uimnote"><?php _e('Redirect users to a page or URL after they successfully submit the group registration form.','profile-magic');?></div>
      </div>
      ...
      <div class="uimrow" id="sucess_message">
        ...
        <div class="uiminput">
          ...
          <div class="uimnote"><?php _e('This message will appear to the user after the form is submitted. If redirection is turned on, it will appear for a few seconds before redirection is triggered.','profile-magic');?></div>
      </div>
       ...
       <div class="uimrow" id="notification">
        <div class="uimfield">
          <?php _e( 'Success Message','profile-magic' ); ?>
        </div>
        <div class="uiminput">
        <?php if(isset($row))$success_message = $row->success_message; else $success_message = ''; wp_editor( $success_message, 'success_message');?>
        </div>
           <div class="uimnote"><?php _e('The contents of the success message. Rich text is supported.','profile-magic');?></div>
      </div>
       </div>
      ...
      <div class="uimrow" id="notification">
        <div class="uimfield">
          <?php _e( 'Member Email Notifications','profile-magic' ); ?>
        </div>
        <div class="uiminput">
          ...
          <div class="uimnote"><?php _e('Send relevant custom email notifications to the user on important registration events. If you have not created any custom email templates, you can save the group now, and come back later to assign them.','profile-magic');?></div>
      </div>
      ...
      <div class="childfieldsrow" id="notification_html" style=" <?php if(!empty($group_options) && isset($group_options['enable_notification']) && $group_options['enable_notification']==1){echo 'display:block;';} elseif($id==0){echo 'display:block;';}else{ echo 'display:none;';} ?>">
       ...
       <div class="uimrow">
        <div class="uimfield">
          <?php _e( 'On Registration','profile-magic' ); ?>
        </div>
        ...
        <div class="uimnote"><?php _e('Sends this message to the user on successful registration form submission.','profile-magic');?></div>
      </div>
      ...
      <div class="uimrow">
        ...
        <div class="uimnote"><?php _e('Sends this message to the user on his/ her user account activation in WP dashboard.','profile-magic');?></div>
      </div>
      ...
      <div class="uimrow">
        ...
        <div class="uimnote"><?php _e('Sends this message to the user when his/ her user account is deactivate in WP dashboard.','profile-magic');?></div>
      </div>
        ...
        <div class="uimnote"><?php _e('Sends this message to the user when his/ her password is changed.','profile-magic');?></div>
      </div>
        ...
        <div class="uiminput <?php if($id==0 || (!empty($group_options) && isset($group_options['enable_notification']) && $group_options['enable_notification']==1)){echo 'pm_select_required';}?>">
          <select name="group_options[on_account_deleted]" id="on_account_deleted">
            ...
            <?php
			  foreach($email_template as $tmpl)
			  {?>
            <option value="<?php echo $tmpl->id;?>" <?php if(!empty($group_options) && isset($group_options['on_account_deleted']) && $group_options['on_account_deleted']==$tmpl->id) echo 'selected'; ?>><?php echo $tmpl->tmpl_name; ?></option>
            <?php }
			  ?>
          </select>
          <div class="errortext"></div>
        </div>
        <div class="uimnote"><?php _e('This email is sent to the user when his/ her account has been deleted from the site.','profile-magic');?></div>
      </div>
        ...
        <div class="uiminput <?php if($id==0 || (!empty($group_options) && isset($group_options['enable_notification']) && $group_options['enable_notification']==1)){echo 'pm_select_required';}?>">
          <select name="group_options[on_published_new_post]" id="on_published_new_post">
            ...
            <?php
			  foreach($email_template as $tmpl)
			  {?>
            <option value="<?php echo $tmpl->id;?>" <?php if(!empty($group_options) && isset($group_options['on_published_new_post']) && $group_options['on_published_new_post']==$tmpl->id) echo 'selected'; ?>><?php echo $tmpl->tmpl_name; ?></option>
            <?php }
			  ?>
          </select>
          <div class="errortext"></div>
        </div>
        <div class="uimnote"><?php _e('Select email to be sent to the users when their blog post is published successfully.','profile-magic');?></div>
      </div>
